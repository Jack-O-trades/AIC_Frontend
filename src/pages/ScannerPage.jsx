...
const inputStyle = {
  ...
  background: '#041c24', border: '1px solid var(--border-color)',
...
const ScannerPage = () => {
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      setSaveMsg({ ok: false, text: 'Save failed. Check connection.' });
    }
    setSaving(false);
  };
      ...
      <div className={`admin-card ${showDataCard ? 'visible' : ''}`} style={{ marginTop: '1.5rem', display: showDataCard ? 'block' : 'none' }}>
        ...
        {saveMsg && (
          <div style={{
            ...
            background: saveMsg.ok ? 'rgba(22,163,74,0.15)' : 'rgba(220,38,38,0.15)',
            border: `1px solid ${saveMsg.ok ? 'rgba(22,163,74,0.4)' : 'rgba(220,38,38,0.4)'}`,
            color: saveMsg.ok ? '#4ade80' : '#f87171',
            fontSize: '0.85rem', fontWeight: '600',
          }}>{saveMsg.ok ? '✓ ' : '✕ '}{saveMsg.text}</div>
        )}
        ...
        {editMode ? (
          ...
          <>
            <div className="data-field">
              ...
              <input style={inputStyle} value={editFields.name}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <div className="data-field">
                ...
                <select style={{ ...inputStyle, cursor: 'pointer' }} value={editFields.role}
              ...
              <div className="data-field">
                ...
                <input style={inputStyle} value={editFields.college}
            ...
            <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1.25rem' }}>
              <button onClick={handleEditSubmit} disabled={saving} style={{
                flex: 1, padding: '0.8rem', borderRadius: '8px', border: 'none',
                background: 'var(--accent-blue)', color: '#041c24',
                fontWeight: '700', fontSize: '0.95rem',
                cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.7 : 1,
              }}>{saving ? 'Saving…' : 'Submit Changes'}</button>
              <button onClick={() => { setEditMode(false); setSaveMsg(null); }} disabled={saving} style={{
          ...
          <>
            <div className="data-field" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                ...
                <div className={`data-value ${scannedData.name === 'Verifying...' ? 'loading' : ''}`}>{scannedData.name}</div>
              </div>
              {scannedData.name !== '-' && scannedData.name !== 'Verifying...' && scannedData.name !== 'Unknown User' && (
                <button onClick={() => { setEditMode(true); setSaveMsg(null); }} style={{
        ...
        {/* Confirm Check-in — hidden while editing */}
